export function WalletDashboard({ balance, frozenBalance, onWithdrawClick }: { balance: number, frozenBalance: number, onWithdrawClick: () => void }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 bg-[#131315] rounded-xl border border-[var(--color-border-700)] p-8 flex flex-col justify-between relative overflow-hidden">
        {/* Glow behind */}
      ...
      <div className="space-y-6">
        <div className="bg-[var(--color-background-900)] p-6 rounded-xl border border-[var(--color-border-700)]">
          ...
          <p className="text-2xl font-extrabold font-headline text-white">{frozenBalance.toLocaleString()} <span className="text-sm text-zinc-500">AC</span></p>
          <p className="text-[10px] text-zinc-500 mt-2">Locked for pending withdrawals</p>
        </div>
