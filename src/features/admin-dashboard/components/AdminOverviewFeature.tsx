...
function getFinanceBarWidth(value: number, maxValue: number) {
  if (value <= 0 || maxValue <= 0) return "0%";
  return `${Math.max((value / maxValue) * 100, 4).toFixed(2)}%`;
}
...
function FinanceTrendChart({ data, locale }: FinanceTrendChartProps) {
  const t = useTranslations("Admin.dashboard");
  ...
  const bars = [
    ...
    {
      label: t("finance.chart.creatorShareLabel"),
      value: creatorShareCoins,
      colorClass: "bg-secondary",
      textClass: "text-secondary",
    },
    {
      label: t("finance.chart.revenueLabel"),
      value: systemRevenueCoins,
  ...
  return (
    <div className="relative overflow-hidden rounded-lg border border-border/20 bg-background/30 p-6 transition-all duration-300">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        ...
        <span className="inline-flex w-fit rounded-sm border border-border/30 bg-card px-2 py-1 font-mono text-[9px] font-bold uppercase tracking-wider text-muted-foreground">
          {t("finance.chart.realDataBadge")}
        </span>
      </div>

      <div className="space-y-4">
        {bars.map((bar) => (
          <div key={bar.label} className="grid gap-2 md:grid-cols-[10rem_1fr_7rem] md:items-center">
            <div className="flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-wider text-zinc-300">
              <span className={`h-2.5 w-2.5 rounded-sm ${bar.colorClass}`} />
              <span>{bar.label}</span>
            </div>
            <div className="h-3 overflow-hidden rounded-full border border-border/20 bg-muted/30">
              <div className={`h-full rounded-full ${bar.colorClass}`} style={{ width: getFinanceBarWidth(bar.value, maxValue) }} />
            </div>
            <span className={`text-right font-mono text-xs font-bold ${bar.textClass}`}>{formatCoins(bar.value, locale)}</span>
          </div>
        ))}
      </div>
    </div>
...
function RevenueDonutChart({ revenue, locale }: RevenueDonutChartProps) {
  const t = useTranslations("Admin.dashboard");
  const creatorShare = revenue.creatorRevenueCoins;
  const systemRevenue = revenue.systemRevenueCoins;
  const total = creatorShare + systemRevenue;
    ...
    <section className="space-y-6 rounded-lg border border-border/30 bg-card p-6">
      ...
      {!isLoading && data && (
        <div className="space-y-6">
          ...
          <FinanceTrendChart data={data} locale={locale} />
